function tes
    ...
    set basedir ~/documents/fac/tesis/cuaderno
    set today $basedir/log/today
    set month (date +'%B')
    if not test -d $basedir/log/$month
        mkdir $basedir/log/$month
    end
    set datefile $basedir/log/$month/(date  +'%d-%m-%Y')
